...
@dynamic pullToRefreshView, showsPullToRefresh, scrollPositionChanged;

- (void)setScrollPositionChanged:(void (^)(CGFloat))scrollPositionChanged {
    self.pullToRefreshView.scrollPositionChanged = scrollPositionChanged;
}
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != SVPullToRefreshStateLoading) {
        ...
        CGFloat percent = (((scrollOffsetThreshold + contentOffset.y) / scrollOffsetThreshold) - 1.0f);
        if (percent < 0) {
            percent = 0;
        }
        
        if (0 <= percent && percent <= 200.f && self.state != SVPullToRefreshStateTriggered) {
            //            NSLog(@"percent: %f", percent);
            if (self.scrollPositionChanged) {
                self.scrollPositionChanged(percent);
            }
        }
        ...
        if(!self.scrollView.isDragging && self.state == SVPullToRefreshStateTriggered)
            ...
            //            NSLog(@"triggered: %f", contentOffset.y);
        }
        ...
        switch (self.position) {
            case SVPullToRefreshPositionTop:
                ...
                // top margin when you release
                offset = MAX(-10 * -1, 0.0f);
